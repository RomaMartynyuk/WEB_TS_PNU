...
export class HistoryController {
    ...
    public loadHistory(): void {
        ...
        sortedHistory.forEach((session: WorkoutSession) => {
            ...
            const volumeFormatted: string = session.totalVolume ? formatVolume(session.totalVolume) : '0 кг';
            ...
            sessionCard.innerHTML = `
                ...
                ${session.exercises.length > 0 ? `
                    <div class="mt-3">
                        <small class="text-muted">Вправи: ${session.exercises.map((ex: Exercise) => {
    ...
    private renderVolumeProgress(currentSession: WorkoutSession, allSessions: WorkoutSession[]): string {
        if (!currentSession.totalVolume || allSessions.length < 2) return '';
        
        // Знаходимо попереднє тренування
        ...
        if (!previousSession.totalVolume) return '';
        ...
        if (Math.abs(percentChange) < 1) return '';
